...
export function parseTrackGpx(
  ...
  if (parseError)
    throw new Error(
      `Invalid GPX: ${parseError.textContent?.trim().slice(0, 120)}`,
    );
  ...
  const allCoords = trackFeatures.flatMap(
    (f) => f.geometry.coordinates as number[][],
  );
  const bbox =
    allCoords.length > 0
      ? bboxFromCoords(allCoords)
      : { west: 0, east: 0, south: 0, north: 0 };

  const displayName =
    extractText(doc, "trk > name") || extractText(doc, "name") || fallbackName;

  const dateStr =
    extractText(doc, "metadata > time") ||
    extractText(doc, "trkpt > time") ||
    null;
...
export function parsePeaksGpx(gpxText: string, category: string): ParsedPeaks {
  ...
  if (parseError)
    throw new Error(
      `Invalid GPX in ${category}: ${parseError.textContent?.trim().slice(0, 120)}`,
    );
  const geojson = gpx(doc) as FeatureCollection;
