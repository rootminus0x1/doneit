...
interface DirEntry { name: string; type: 'file' | 'directory' }
...
export async function findOrCreateFolder(
  _token: string | null, name: string, parentId: string
): Promise<string> {
  return parentId ? `${parentId}/${name}` : name
}

export async function listFolders(
  _token: string | null, parentId: string
): Promise<DriveFile[]> {
...
export async function readFileText(
  _token: string | null, fileId: string
): Promise<string> {
...
export async function upsertJsonFile(
  _token: string | null, name: string, parentId: string, content: unknown
): Promise<void> {
