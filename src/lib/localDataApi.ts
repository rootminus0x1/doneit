...
export async function findOrCreateFolder(_token: string | null, name: string, parentId: string): Promise<string> {
    return parentId ? `${parentId}/${name}` : name;
}

export async function listFolders(_token: string | null, parentId: string): Promise<DriveFile[]> {
...
export async function listFiles(
    ...
    return entries
        ...
        .map(e => ({
            id: parentId ? `${parentId}/${e.name}` : e.name,
            name: e.name,
            mimeType: e.name.endsWith('.gpx') ? 'application/gpx+xml' : 'application/json',
        }));
}

export async function readFileText(_token: string | null, fileId: string): Promise<string> {
...
export async function findFileByName(_token: string | null, name: string, parentId: string): Promise<DriveFile | null> {
...
export async function upsertJsonFile(
    ...
    localStorage.setItem(`local:${parentId}/${name}`, JSON.stringify(content, null, 2));
}
